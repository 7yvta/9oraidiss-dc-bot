...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("amount")
        .setDescription("Coins to deposit (number or all)")
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    const maxMovable = Number(before.wallet || 0);
    ...
    const account = await updateAccount(interaction.guild.id, interaction.user.id, async (acc) => {
      if (Number(acc.wallet || 0) < amount) return;
      acc.wallet -= amount;
      acc.bank = Number(acc.bank || 0) + amount;
      ok = true;
    });
      ...
      return interaction.reply({
        embeds: [
          buildResultEmbed({
            ...
            color: 0xed4245,
            description: "You do not have that much in your wallet."
          })
        ],
        flags: MessageFlags.Ephemeral
      });
    ...
    await interaction.reply({
      embeds: [
        economyEmbed({
          title: `Deposited ${coins(amount)} to your bank. Bank: ${coins(account.bank)}`,
          color: SUCCESS_COLOR,
          footer: null,
          timestamp: false
        })

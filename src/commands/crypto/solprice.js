...
module.exports = {
  data: new SlashCommandBuilder().setName("solprice").setDescription("Live SOL price, 24h change, and market cap"),
  async execute(interaction) {
    const price = await fetchCryptoPrice("sol");
    await interaction.reply({ embeds: [economyEmbed({ title: "? Solana Price", fields: [

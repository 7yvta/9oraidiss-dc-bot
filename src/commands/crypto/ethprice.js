...
module.exports = {
  data: new SlashCommandBuilder().setName("ethprice").setDescription("Live ETH price, 24h change, and market cap"),
  async execute(interaction) {
    const price = await fetchCryptoPrice("eth");
    await interaction.reply({ embeds: [economyEmbed({ title: "? Ethereum Price", fields: [
